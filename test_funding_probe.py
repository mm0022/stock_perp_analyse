...
# ---- _init_output / _Tee：跨平台编码，替代原先的 .bat 包装 ----


def test_init_output_survives_gbk_stream(capsys):
    """Windows 中文系统重定向用 GBK，emoji 编不出来会崩。
    _init_output 设 errors='replace' 后：中文完好、emoji 降级为 ?、不崩。"""
    import io
    import sys
...
def test_tee_appends_not_truncates(tmp_path):
    """长跑重启后日志要接着写，不能截断"""
    import io
    p = tmp_path / "t.log"
    p.write_text("旧内容\n", encoding="utf-8")
    tee = fp._Tee(io.StringIO(), str(p))
    tee.write("新内容\n")
    tee.flush()
    tee.file.close()
